...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Social Media Scraping</h1>
            <p class="mt-2 text-sm text-gray-700">Scrape data from various social media platforms using snscrape</p>
        </div>
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Supported Platforms</h3>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($supportedPlatforms as $platform => $scrapers)
                <div class="bg-gray-50 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-3">
                        <h4 class="text-lg font-medium text-gray-900 capitalize">{{ $platform }}</h4>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            Available
                        </span>
                    </div>
                    <div class="space-y-2">
                        @foreach($scrapers as $type => $scraper)
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-600 capitalize">{{ $type }}</span>
                            <code class="text-xs bg-gray-200 px-1 rounded">{{ $scraper }}</code>
                        </div>
                        @endforeach
                    </div>
                </div>
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <div class="sm:flex sm:items-center sm:justify-between mb-4">
                ...
                <a href="{{ route('dashboard.jobs') }}" class="text-sm text-indigo-600 hover:text-indigo-500">View all jobs</a>
            </div>
